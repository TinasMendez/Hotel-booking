...
test("should open a product and guard reservation when not logged in", async ({ page }) => {
    await page.goto("/");
    ...
    const reserveBtn = page.getByRole("button", { name: /reserve|book now|reserve now/i }).first();
