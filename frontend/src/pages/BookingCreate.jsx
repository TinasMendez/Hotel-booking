...
export default function BookingCreate() {
    ...
    useEffect(() => {
        ...
        try {
            ...
            if (!cancelled) setError("Invalid product identifier. Please return to the catalog.");
        } finally {
    ...
    async function create() {
        ...
        const msg = e?.response?.data?.message || e?.message || "Unable to create booking. Please try again.";
        setError(msg);
        } finally {
        setSubmitting(false);
        }
    ...
    return (
        ...
        <section className="grid">
            ...
            <h2>Property</h2>
            <div className="prod">
                <img src={product?.imageUrl || product?.imageUrls?.[0]} alt={product?.name} />
                <div>
                <b>{product?.name}</b>
                <p className="muted">{product?.description?.slice(0, 120) || "—"}</p>
                <Link to={`/product/${product?.id}`} className="link">View property</Link>
                </div>
            </div>
            </article>
            ...
            <h2>Guest</h2>
            <ul className="list">
                <li><span>Name</span><b>{(user?.firstName || "") + " " + (user?.lastName || "")}</b></li>
